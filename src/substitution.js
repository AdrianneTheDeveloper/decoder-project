...
function substitution(input, alphabet, encode = true) {
    ...
    try {
        ...
        const checkForRepeated = alphabet.toLowerCase().split("").sort().join("").match(/(.)\1+/g); // Check for duplicate characters
        if (checkForRepeated !== null) throw `Alphabet must not have duplicate letters.` // If checkForRepeated is empty, throw an error
    }
    catch (error) {
        return error
    }
    ...
    const origAlpha = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"]; // Declare array for actual alphabet
    ...
    if (encode === true && !/\s/.test(input)) { // If encode is true and there are no spaces in input
        for (let inputInd = 0; inputInd < inputArr.length; inputInd++) { // Loop through inputArr
            for (let cypherInd = 0; cypherInd < cypher.length; cypherInd++) { // Loop through cypher
    ...
    } else if (encode === true && /\s/.test(input)) { // If encode is true and there are spaces in input
       ...
       let inputArr2 = input.split(' ') // Split string inside array into new array with one space in between each letter [[s,t,r,i,n,g], [s,t,r,i,n,g]]
        ...
        inputArr2.forEach((element => { // Loop through each element in array
            let encoded = []; // Declare variable to push each encoded string into
            for (let wordInd = 0; wordInd < element.length; wordInd++) {  // Loop through each word 
                for (let cypherInd = 0; cypherInd < cypher.length; cypherInd++) { // Loop through cypher
                    
                    let key = Object.keys(cypher[cypherInd]); // Declare variable for object key
                    if (element[wordInd] == Object.keys(cypher[cypherInd])) encoded.push(cypher[cypherInd][key]) // If word index matches object key index, push corresponding property into encoded array
        ...
        inputArr2.forEach((element => { // Loop through each element in array
            ...
            for (let wordInd = 0; wordInd < element.length; wordInd++) {  // Loop through each word 
                for (let cypherInd = 0; cypherInd < cypher.length; cypherInd++) { // Loop through cypher
                    ...
                    let key = Object.keys(cypher[cypherInd]); // Declare variable for object key
                    if (element[wordInd] == cypher[cypherInd][key]) encoded.push(Object.keys(cypher[cypherInd])) // If word index matches object property index, push corresponding key into encoded array
